...
cP_t3 = [cP00_t1', cP01_t1', cP02_t1', cP03_t1', cP10_t1', cP11_t1', cP12_t1', cP13_t1', cP20_t1', cP21_t1', cP22_t1', cP23_t1', cP30_t1', cP31_t1', cP32_t1', cP33_t1'];
         ...
         for j = 1:numel(A)
             for k = 1:numel(A)
                 for l = 1:numel(A)
                     ...
                     % C4term_val =  A(l) * A(k) * A(j) * A(i) * cP_t2(l) .* cP_prod(j,k) * Peq(i);
                     C4term_val_almost =  A(l) * A(k) * A(j) * A(i) * cP_t2(k,j) * Peq(i);
                     ...
                     C4vec_almost = [C4vec_almost; C4term_val_almost];
 ...
 % end
 disp('Time to calculate sum over As cP_t2 and Peq is')
 toc
 
 tic
 C4mat = [];
 for i = 1:length(cP_prod)
     for j = 1:length(cP_prod)
         C4vec_mat = C4vec_almost * cP_prod(i,j);
         %C4mat(i,j) = C4vec_mat(i,j);
         C4mat = [C4mat, C4vec_mat];
     end
 end
 
 
 C4matrix = reshape(C4mat,[256,50,50]);
 C4 = sum(C4matrix, 1);
 C4 = squeeze(C4);
 
 disp('Time to calculate C4 matrix is')
 toc
