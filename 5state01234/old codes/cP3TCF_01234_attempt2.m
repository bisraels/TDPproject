...
elseif double(C2(10^20)) < 1e-20 && double(sqm) < 1e-20
    disp('Both C2(t = big) and <A>^2 (sqm) are less than 1e-20, so they match close enough to zero.')
else 
...
%--------------------------------------------------------------------------
figure
TCF4point = surf(tau1vec(2:end), tau3vec(2:end), C4(2:end,2:end));
